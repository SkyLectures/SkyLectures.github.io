...
### 1.1 데이터 중심 아키텍처로의 전환

- **목적:**
    - 전통적인 요청-응답(Request-Response) 모델, 배치(Batch) 위주의 데이터 처리에서 벗어나,
    - 모든 이벤트를 실시간 흐름으로 파악하는 아키텍처 이해

- **의의:**
    - 비즈니스에서 발생하는 사건(Event)을 즉각적으로 가공하여 가치를 창출할 수 있는
    - 현대적 데이터 기반 의사결정의 토대 마련

### 1.2 시스템 간 결합도 해제 및 확장성 확보

- **목적:**
    - Kafka를 중앙의 메시지 중추(Backbone)로 활용하여
    - 소스 시스템(DB)과 타겟 시스템(Storage) 간의 의존성을 분리하는 방법의 학습
...
>   - CDC와 Kafka Connect의 조합을 성공적으로 수행한다면, 기업 현장의 레거시 시스템을 현대화하는 핵심 엔지니어로서의 자질을 증명하게 될 것
{: .summary-quote}