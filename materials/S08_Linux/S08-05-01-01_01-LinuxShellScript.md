...
2. **💡 상세 설명 (핵심 포인트)**
    - **Shebang (#!/bin/bash)**
        ...
        > - **Shebang**
        >   - 전부, 모두 라는 의미
        >   - 셔뱅, 샤-뱅(sha-bang), 해시뱅(hashbang), 파운드-뱅(pound-bang), 해시-플링(hash-pling), 크런치뱅(crunchbang)이라고도 함
        >   - 해시 기호와 느낌표(#!) 로 이루어진 문자 시퀀스로, 스크립트의 맨 처음에 옴
        >   - 스크립트를 실행할 때 어떤 인터프리터(interpreter)를 사용할지를 운영체제에게 알려주는 역할
        >       - Unix계열 OS(리눅스, Mac)에서 스크립트(bash, python등등) 코드 최상단에서 해당 파일을 해석해줄 인터프리터의 절대경로를 지정함
        >   - 다음과 같은 구조를 가짐
