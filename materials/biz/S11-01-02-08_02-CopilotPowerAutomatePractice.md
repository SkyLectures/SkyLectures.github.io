...
- **전체 프로세스**
    1. **[Excel Copilot 트리거]**
        - `시장_판매_데이터_2025.xlsx` 파일에서 Excel Copilot에게 다음과 같이 요청
    ...
    3. **[Power Automate ➜ GPT]**
        ...
        - GPT에는 다음 프롬프트가 전송될 수 있음

        ```
        #Role: 시장 분석 전문가 및 전략 컨설턴트

        아래 제공된 [시장 판매 데이터]와 [경쟁사 분석 데이터]를 기반으로 다음을 수행해 줘:
        1. 라즈베리파이 엣지 AI 시장의 최근 6개월간 주요 동향 3가지를 도출하고, 각각에 대한 간략한 설명을 추가해 줘.
        2. 우리 제품 '라즈베리파이 엣지 AI 세그멘테이션 솔루션'의 현재 시장점유율 (가정: 5%) 및 잠재성장률을 분석하고,
           주요 경쟁사 대비 강점 3가지와 약점 2가지를 설명해 줘.
        3. 이 분석을 바탕으로, '시장 분석 및 전략 제안' PowerPoint 슬라이드 3개(제목 및 핵심 내용)의 초안을 제시해 줘.
...
- **전체 프로세스**
    1. **[Outlook Copilot 트리거]**
        ...
        - Outlook Copilot에게 "이 메일 내용을 분석하여 핵심 기술 문제와 가능한 해결책을 파출하고, 이를 Jira에 기술 지원 티켓으로 생성해 줘."라고 요청
    ...
    3. **[Power Automate ➜ GPT]**
        ...
        3. 문제 해결을 위한 '예상 해결책' 3가지(또는 그 이하)를 간략히 제시해 줘. 
           (예: 펌웨어 업데이트, 설정 변경, 하드웨어 교체)
        4. '문제 관련 키워드' 3~5개를 추출해 줘.
           (예: 라즈베리파이 5, 세그멘테이션, 성능 저하)
