...
- **데이터 분석 파이프라인 단계별 설명**
    - **데이터 탐색 (`info()`, `describe()`)**
        - 데이터를 받자마자 무작정 연산을 시작하지 말것
        - `df.info()`를 통해 각 컬럼의 데이터 타입과 결측치 존재 여부 파악
        - `df.describe()`를 통해 평균, 최솟값, 최댓값, 사분위수 등 데이터의 전반적인 분포와 이상치 징후 확인

    - **데이터 정제 (Data Cleaning)**
        - **분석의 걸림돌 제거:**
            - `Youtube`와 `youtube`는 데이터 분석 시 서로 다른 그룹으로 분리되는 문제가 있음 🡲 `.str.upper()`로 모두 `YOUTUBE`로 통일
        - **타입 매칭:**
            - 연산이 불가능한 쉼표 텍스트 형태의 매출액 데이터를 정수형(`int`)으로 변환
            - 데이터가 비어 있는 `Impressions` 항목은 기존 데이터의 평균값으로 메워 데이터 손실 방지

    - **파생 변수 생성 (Feature Engineering)**
        - 원본 데이터에 있는 숫자 자체보다, 데이터 간의 관계를 통해 새로운 의미를 도출하는 단계
        - 노출 대비 클릭 수의 비율인 CTR(클릭률) 계산 🡲 어떤 채널이 고객의 이목을 가장 잘 끌었는지 파악할 수 있는 정량적 기준 마련

    - **그룹화 및 정렬 (`groupby()`, `sort_values()`)**
        - 데이터 분석의 핵심은 **비교와 요약**
        - `groupby('Channel')`를 사용하면 각 채널(FACEBOOK, INSTAGRAM, YOUTUBE) 별로 데이터를 쪼개어 합계(`sum`)나 평균(`mean`)을 연산할 수 있음
        - `sort_values()`를 사용하여 가장 매출 기여도가 높은 채널이 무엇인지 한눈에 보이도록 데이터를 정렬