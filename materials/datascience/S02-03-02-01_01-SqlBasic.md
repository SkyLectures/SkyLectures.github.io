...
제시해주신 DML의 핵심 명령어들에 대하여, 수강생들이 데이터의 생성부터 조회까지의 라이프사이클을 명확히 이해하고 쿼리를 정확히 작성할 수 있도록 핵심 설명, 표준 구문 형식, 그리고 구체적인 실습 예제(앞서 정의한 `products` 테이블 기준)를 매칭하여 깔끔하게 보완해 드립니다.
