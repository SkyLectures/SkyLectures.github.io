...
- Streamlit에서 제공하는 세 가지 핵심 개념
    - **세션 상태 (Session State - `st.session_state`):**
        - Streamlit은 사용자가 상호작용(버튼 클릭, 텍스트 입력 등)을 할 때마다 코드를 처음부터 다시 실행함
        - 이전 대화 내용을 유지하려면 `st.session_state`라는 딕셔너리 객체에 메시지 기록을 저장하여 상태를 보존해야 함

    - **채팅 요소 (Chat Elements - `st.chat_message`, `st.chat_input`):**
        - `st.chat_input`: 화면 하단에 고정되는 채팅 입력창 제공
        - `st.chat_message`: 사용자(User)와 봇(Assistant)의 메시지를 시각적으로 구분하여 화면에 렌더링하는 컨테이너

    - **스트리밍 출력 (Streaming Output - `st.write_stream`):**
...
- 효율적인 테스트와 사용을 위해 화면을 두 가지 주요 영역으로 나눔
    - **사이드바 (Sidebar):**
        - 챗봇의 동작을 제어하는 설정 영역
        - **역할:**
            - 시스템 프롬프트 설정
            - 모델 선택(예: Local vLLM, API 등)
            - Temperature 등 하이퍼파라미터 조절

    - **메인 화면 (Main Area):**
        - 실제 대화가 이루어지는 영역
    - **역할:**
        - 상단: 인사말 또는 제목
        - 중간: 누적된 대화 기록(`st.session_state` 기반 렌더링)
        - 하단: 채팅 입력창 배치
